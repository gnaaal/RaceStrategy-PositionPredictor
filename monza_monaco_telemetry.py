...
# stratify=y ensures the class distribution in the training and testing sets is the same as in the original dataset, which is important for imbalanced datasets to ensure that both sets have a representative distribution of the target classes.
X_train, X_test, y_train, y_test = train_test_split(X_scaled, y, test_size=0.2, random_state=42, stratify=y)

# Model initialization 
# Parameter class_weight='balanced' is very important to insure the model gives more attention to the minority class (Naik and Turun) during training,
# especially since the dataset is imbalanced with a majority of 'Tetap' samples. This helps improve the model's ability to correctly classify the minority classes, which are crucial for our analysis.
...
# asking the trained models to predict the class labels for the test set (X_test) and storing the predictions in y_pred_rf and y_pred_svm respectively.
y_pred_rf = rf_model.predict(X_test)
y_pred_svm = svm_model.predict(X_test)

# Evaluate Model Performance
# Showing the accuracy and classification report for both models to evaluate their performance in predicting the target classes (Naik, Tetap, Turun) based on the test set.
